...
class Rate extends CI_Controller {
    ...
    public function index() {
        isLoggedIn();
        $data['model'] = $this->Rate_model->get_all();
    ...
    public function edit($id = null) {
        ...
        $data = array();
        $data['page'] = $this->_page;
        $data['data'] = $this->Rate_model->get_by_id($this->uri->segment(3));
        
        if ($this->input->post()) {
            $myData = $this->input->post();            
            $myData['id'] = $this->uri->segment(3); 
            $this->Rate_model->update($myData);
    ...
    public function delete($id = null) {
        isLoggedIn();
        $this->Rate_model->delete_by_id($id);
